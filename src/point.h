...
template <typename T> struct Point3 {
  ...
  inline Point3<T> cross(const Point3<T> &rhs) const {
    return Point3<T>{y * rhs.z - z * rhs.y, -x * rhs.z + z * rhs.x,
                     x * rhs.y - y * rhs.x};
  }
...
template <typename T> struct Point4 {
  ...
  Point4<T>(const T &_x = (T)0, const T &_y = (T)0, const T &_z = (T)0,
            const T &_w = (T)0)
      : x(_x), y(_y), z(_z), w(_w) {}
  ...
  inline Point4<T> operator-() const { return Point4<T>{-x, -y, -z, -w}; }

  operator Point3<T>() const { return Point3<T>{x, y, z}; }
