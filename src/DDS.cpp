...
bool ValidateFlags(const DDSHeader &header) {
  return (header.flags & DDSD_CAPS) != 0 && (header.flags & DDSD_HEIGHT) != 0 &&
...
bool LoadDDS(const std::wstring &filepath, TextureDesc &desc, bool singleMip) {
  ...
  // Read pitch
  desc.pitch =
      (header.flags & DDSD_PITCH) != 0 ? (UINT32)header.pitchOrLinearSize : 0;

  // Read mipmap count
  desc.mipmapsCount =
      (header.flags & DDSD_MIPMAPCOUNT) != 0 ? (UINT32)header.mipMapCount : 1;
  ...
  // Get data size
  UINT32 dataSize = (header.flags & DDSD_LINEARSIZE) != 0
                        ? (UINT32)header.pitchOrLinearSize
                        : 0;
  if (dataSize == 0) {
